...
import { getClassDisplayName } from "@/utils/school/classData";

const ClassDeleteDialog = ({ open, classItem, loading = false, onClose, onConfirm }) => (
  <Dialog
    open={open}
  ...
  >
    ...
    <DialogContent sx={{ px: 2.4, py: "24px !important", textAlign: "center" }}>
      ...
      <Typography sx={{ color: "#122f4d", fontSize: "13px", fontWeight: 800 }}>
        حذف "{getClassDisplayName(classItem)}"؟
      </Typography>
      <Typography sx={{ mt: 0.8, color: "#7e8791", fontSize: "9px", lineHeight: 1.8 }}>
        سيتم حذف الفصل نهائيًا. تأكد من إزالة الطلاب والعلاقات المرتبطة به قبل الحذف.
      </Typography>
    </DialogContent>
    ...
    <DialogActions sx={{ px: 2.4, py: 1.5, gap: 0.8, borderTop: "1px solid #ded8cd" }}>
      ...
      >
        {loading ? <CircularProgress size={16} sx={{ color: "inherit" }} /> : "تأكيد الحذف"}
      </Button>
    </DialogActions>
  </Dialog>
);
