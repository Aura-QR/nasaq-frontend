...
const Input = ({label , required , error , registerName , register , type , defaultValue , multiline, valueAsNumber}) => {
  ...
  const openPicker = () => {
    if (inputRef.current?.showPicker && type === "date") {
      inputRef.current.showPicker(); // Open the native date picker
    }
  };
  
  return (
    <Stack alignItems={"start"} spacing={4} width={"100%"}>
        ...
        <TextField 
          ...
          {...register(registerName , { 
            ...
            pattern : (type === "email" || type === "password") && {
              value : type === "email" ? /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i : /^(?=.*[a-z])(?=.*\d).{8,}$/,
              message : type === "email" ? "Please enter a valid email" : "Password must contain at least one uppercase letter , one lowercase letter and one number"
            },
          ...
          rows={4}
          inputProps={{
            ...(type === "date" && { dir: "ltr", style: { textAlign: "right" } }),
          }}
          sx={{
