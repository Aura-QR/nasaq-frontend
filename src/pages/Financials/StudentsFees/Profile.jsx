...
const FinancialRecordProfilePage = () => {
  ...
  const installmentsRows = useMemo(() => {
    ...
    return (financialRecord?.tuition?.installments || []).map((item) => ({
      ...
      paymentDate:
        item?.status === "paid" && item?.payments?.length > 0 && item?.payments[item?.payments?.length - 1]?.paidAt
  ...
  const handlePayInstallment = async (formData) => {
    ...
    if (response.status) {
      ...
      toast.error(response || "حدث خطأ ما أثناء تسجيل دفعة القسط");
    }
    setPayLoading(false);
  };
  ...
  const effectiveFee = Number(tuition?.fee || 0);
