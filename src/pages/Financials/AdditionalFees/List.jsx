...
const getTargetLabel = (item) => {
  const typeLabel = TARGET_LABELS[item?.targetType] || "غير محدد";

  if (item?.targetType === "academicYear") {
  ...
  if (target && typeof target === "object") {
    const targetName =
      target?.name ||
      [target?.firstName, target?.fatherName, target?.familyName]
        .filter(Boolean)
        .join(" ") ||
      target?.roomNumber ||
      target?.email;
...
const AdditionalFeesListPage = () => {
  ...
  const list = additionalFees.length > 0 ? additionalFees : fees;
  const updateList = setAdditionalFees || setFees;
  ...
  const allStudentsCount = list.filter(
    (item) => item?.targetType === "all" || item?.targetType === "school"
  ).length;

  const targetedCount = list.length - allStudentsCount;

  const handleDelete = async (id, setOpen) => {
    if (!permissions?.delete) {
      toast.error("ليس لديك صلاحية حذف الرسوم الإضافية");
      return;
    }

    const response = await deleteAdditionalFee(id);

    if (response?.status) {
      toast.success(
        response?.message || "تم حذف الرسم الإضافي بنجاح"
      );
    ...
    toast.error(
      response?.message ||
        (typeof response === "string" ? response : "حدث خطأ أثناء حذف الرسم الإضافي")
    );
  };
      ...
      <Box dir="rtl" sx={{ pb: 4 }}>
        ...
        >
          {!loading && rows.length === 0 ? (
            <EmptyState
              icon={<PostAddRounded />}
              title={error || "لا توجد رسوم إضافية حتى الآن"}
