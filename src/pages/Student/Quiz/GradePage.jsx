...
import { Cancel, CheckCircle, RateReview, TaskAlt } from "@mui/icons-material";
...
const GradePage = () => {
  ...
  const { gradeResult, isTimeUp, questions, answers, examTitle } = location.state || {};
  ...
  const title = examTitle || "نتيجة الكويز";

  // Derive list path from current URL (/exams or /assignments)
  const listPath = location.pathname.includes("assignments")
      ...
      <div className="mt-6 min-h-[calc(100vh-200px)] flex items-center justify-center">
        <div className="w-full max-w-2xl rounded-3xl border border-gray-200 bg-white shadow-sm p-8 text-center">
          ...
          <div
            ...
            style={{ backgroundColor: passed ? "#ECFDF5" : "#FEF2F2" }}
          >
            <TaskAlt className="text-4xl" style={{ color: passed ? "#10b981" : "#ef4444" }} />
          </div>

          <h2 className="text-2xl font-extrabold text-[#1E293B] mb-2">تم تسليم الكويز</h2>
          <p className="text-sm text-gray-500 mb-6">
            {isTimeUp ? "انتهى الوقت وتم التسليم تلقائيا." : "تم حفظ إجاباتك بنجاح."}
          </p>
          ...
          <div className="flex justify-center mb-5">
            ...
            >
              {passed ? <CheckCircle className="text-base" /> : <Cancel className="text-base" />}
              {passed ? "ناجح" : "راسب"}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            {questions?.length > 0 && (
              <button
                type="button"
                onClick={() => {
                  // Build absolute path — relative "../review" breaks with flat routes
                  const reviewPath = location.pathname.replace(/\/[^/]+$/, "/review");
                  navigate(reviewPath, {
                    state: { gradeResult, questions, answers, examTitle },
                  });
                }}
